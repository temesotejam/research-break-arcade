...
const NODE_TYPES={
  ...
  ARTILLERY:{label:"砲撃拠点",mark:"砲",color:"#d86b6b",desc:"遠距離から敵ユニットを砲撃"},
  DEFENSE:{label:"防衛拠点",mark:"防",color:"#8fc7d9",desc:"接近した敵へ高頻度射撃"}
};
...
function chooseBuildType(side){
  ...
  const scores={
    ...
    DEFENSE:.80+f.caution*1.05+f.coordination*.30+((f.doctrine==="DEFEND"||f.doctrine==="REGROUP") ? 1.20 : 0)+Math.min(1.6,m.nearBase*.24)+(m.hurt>.28?.45:0)+(m.defenseNodes===0?.25:0)
  };
...
function tryBuildFactionNode(side){
  ...
  const type=chooseBuildType(side),cost=type==="ARTILLERY"?12:(type==="DEFENSE"?11:9);
  ...
  nodes.push({
    ...
    fireCooldown:type==="ARTILLERY"?rnd(1.0,2.2):(type==="DEFENSE"?rnd(.15,.40):0)
  });
...
function nodeStrategicValue(side,n){
  ...
  if(n.type==="DEFENSE")v+=.58+f.caution*.60+f.coordination*.25+((f.doctrine==="DEFEND"||f.doctrine==="REGROUP") ? .55 : 0);
...
function chooseArtilleryTarget(node){
  ...
  const candidates=enemies(node.owner).filter(u=>Math.hypot(u.x-node.x,u.y-node.y)<=235);
  let best=null,bestScore=-Infinity;
  for(const u of candidates){
    const hp=u.hp/u.maxHp;
    const local=enemies(node.owner).filter(e=>Math.hypot(e.x-u.x,e.y-u.y)<45).length;
...
function fireArtillery(node,target){
  const damage=14+rnd(-2,3);
  target.hp-=damage;
  shotEffects.push({
    x1:node.x,y1:node.y,x2:target.x,y2:target.y,
    side:node.owner,life:.20
  });
}
function chooseDefenseTarget(node){
  if(!node.owner)return null;
  const candidates=enemies(node.owner).filter(u=>Math.hypot(u.x-node.x,u.y-node.y)<=120);
  let best=null,bestScore=-Infinity;
  for(const u of candidates){
    const d=Math.hypot(u.x-node.x,u.y-node.y);
    const hp=u.hp/u.maxHp;
    const score=(1-hp)*20-d*.18+(u.role==="LINE"?3:0)+rnd(-1.5,1.5);
    if(score>bestScore){bestScore=score;best=u}
  }
  return best;
}
function fireDefense(node,target){
  const damage=3.0+rnd(-.5,.8);
  target.hp-=damage;
  shotEffects.push({
    x1:node.x,y1:node.y,x2:target.x,y2:target.y,
    side:node.owner,life:.09,defense:true
  });
}
    ...
    if(n.type==="ARTILLERY"){
      ...
      if(n.fireCooldown<=0){
        ...
        if(target){
          ...
          n.fireCooldown=rnd(3.0,4.2);
        }else n.fireCooldown=rnd(.9,1.5);
      }
    }else if(n.type==="DEFENSE"){
      n.fireCooldown=(n.fireCooldown??rnd(.15,.40))-dt;
      if(n.fireCooldown<=0){
        const target=chooseDefenseTarget(n);
        if(target){
          fireDefense(n,target);
          n.fireCooldown=rnd(.34,.52);
        }else n.fireCooldown=rnd(.18,.30);
      }
    }
  }
  for(const side of ["EMBER","AZURE"]){
...
function drawNodes(){
  for(const n of nodes){
    ...
    if(n.type==="ARTILLERY"){ctx.strokeStyle=nt.color;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(n.x,n.y-6);ctx.lineTo(n.x+8,n.y-13);ctx.stroke()}
    if(n.type==="DEFENSE"){ctx.strokeStyle=nt.color;ctx.lineWidth=1.5;ctx.beginPath();ctx.arc(n.x,n.y,15,0,Math.PI*2);ctx.stroke()}
...
function updateUI(){
  ...
  for(const side of ["EMBER","AZURE"]){
    ...
    (side==="EMBER"?emberMemory:azureMemory).textContent=f.memory+" · 補給 "+f.supply.toFixed(0)+" · 撃破/損失 "+f.kills+"/"+f.losses+" · 拠点 "+nodeAbilitySummary(side)+" · 建設 "+(f.builtNodeType?NODE_TYPES[f.builtNodeType].label:"検討中");
  }
}
