...
function updateDragonfly(dt,now){const daylight=getLight()>.48;dragonfly.active=daylight&&rainTimer<=0;if(!dragonfly.active){if(viewMode==="dragonfly"){viewMode="pond";viewIndex=0;updateViewLabel();showMessage("トンボは帰っていきました。");}return;}dragonfly.timer-=dt;if(dragonfly.timer<=0||Math.hypot(dragonfly.tx-dragonfly.x,dragonfly.ty-dragonfly.y)<12){dragonfly.tx=rnd(W*.16,W*.84);dragonfly.ty=rnd(H*.15,H*.40);dragonfly.timer=rnd(2,4.5)}dragonfly.x+=(dragonfly.tx-dragonfly.x)*dt*.55;dragonfly.y+=(dragonfly.ty-dragonfly.y)*dt*.55+Math.sin(now*.014)*.1}
...
function updateCamera(dt){
 ...
 if(viewMode==="fish"){const f=fish[0];tx=f.x;ty=f.y;ts=2.15;ta=-Math.PI/2-Math.atan2(f.vy,f.vx)}
...
function drawReeds(l){ctx.strokeStyle=l>.5?"#526f45":"#3a5144";ctx.lineWidth=3;for(let i=0;i<28;i++){const a=i/28*6.28,x=pond.cx+Math.cos(a)*(pond.rx+25),y=pond.cy+Math.sin(a)*(pond.ry+20);ctx.beginPath();ctx.moveTo(x,y);ctx.quadraticCurveTo(x+Math.cos(a)*8,y-24,x+Math.cos(a)*14,y-52-(i%6)*3);ctx.stroke()}}
