...
function makeFaction(name,traits){
  return{
    name,...traits,
...
function balancedTraitPair(){
  ...
  const ember={},azure={};
  keys.forEach((k,i)=>{ember[k]=base+d[i];azure[k]=base-d[i]});
  return{ember,azure};
}
...
function reset(){
  ...
  factions={EMBER:makeFaction("EMBER",pair.ember),AZURE:makeFaction("AZURE",pair.azure)};
  createNodes();
  for(let i=0;i<18;i++){spawnUnit("EMBER",true);spawnUnit("AZURE",true)}
  factions.EMBER.supply=20;factions.AZURE.supply=20;
  renderTraits();log("新しい戦局を開始 · 総合特性を揃え、長所と短所を再生成");
...
function updateNodes(dt){
  for(const n of nodes){
    ...
    if(n.capture>3.5){n.owner="EMBER";n.capture=0;log("エンバーが拠点 "+n.id+" を確保")}
    else if(n.capture<-3.5){n.owner="AZURE";n.capture=0;log("アズールが拠点 "+n.id+" を確保")}
  }
}
...
function checkEnd(){
  for(const side of ["EMBER","AZURE"]){
    ...
    if(pressure>5.4&&pressure>def*1.6&&living(enemy).length<8){
      statusText.textContent=(side==="EMBER"?"エンバー":"アズール")+" 優勢";running=false;log((side==="EMBER"?"エンバー":"アズール")+" が敵司令地域を制圧");
    }
  }
}
...
function drawLabels(){
  ...
  ctx.fillText("エンバー拠点",28,H/2-30);
  ctx.textAlign="right";ctx.fillText("アズール拠点",W-28,H/2-30);ctx.textAlign="left";
}
...
function updateUI(){
  ...
  for(const side of ["EMBER","AZURE"]){
    ...
    (side==="EMBER"?emberMemory:azureMemory).textContent=f.memory+" · 補給 "+f.supply.toFixed(0)+" · 撃破/損失 "+f.kills+"/"+f.losses;
  }
}
