...
const NODE_TYPES={
  ...
  DEFENSE:{label:"防衛要塞",mark:"防",color:"#8fc7d9",desc:"退避・防御・回復の拠点"}
};
...
function defenseAuraMultiplier(side,p){
  return nodes.some(n=>n.owner===side&&n.type==="DEFENSE"&&Math.hypot(n.x-p.x,n.y-p.y)<105)?.72:1;
}
...
function reviewGarrisons(side){
  ...
  if(!owned.length){
    for(const u of living(side))u.garrisonNode=null;
    f.nextGarrisonReview=simTime+rnd(1.8,2.8);
    return;
  }
  ...
  for(const slot of slots){
    ...
    for(const u of living(side)){
      ...
      const existingBonus=u.garrisonNode===slot.node?-42:0;
      const cost=Math.hypot(u.x-slot.node.x,u.y-slot.node.y)+rolePenalty+existingBonus;
...
function assignUnit(u){
  ...
  if(hp<.25+.25*f.caution){
    u.mode="RETREAT";
    u.target=nearestOwnedDefense(u.side,u)||f.base;
    return;
  }
...
function updateNodes(dt){
  for(const n of nodes){
    ...
    const fortressResistance=(n.type==="DEFENSE"&&n.owner&&n.owner!==winner)?.52:1;
...
function fireArtillery(node,target){
  ...
  target.hp-=damage*defenseAuraMultiplier(target.side,target);
  shotEffects.push({
...
function updateNodeAbilities(dt){
  ...
  for(const side of ["EMBER","AZURE"]){
    ...
    for(const u of living(side)){
      ...
      for(const n of forts)if(Math.hypot(u.x-n.x,u.y-n.y)<100)heal+=2.0;
