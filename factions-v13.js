...
function updateUI(){
  ...
  for(const side of ["EMBER","AZURE"]){
    ...
    (side==="EMBER"?emberMemory:azureMemory).textContent=f.memory+" · 補給 "+f.supply.toFixed(0)+" · 撃破/損失 "+f.kills+"/"+f.losses+" · 拠点 "+nodeAbilitySummary(side)+" · 建設 "+(f.builtNodeType?NODE_TYPES[f.builtNodeType].label:"検討中")+" · 要塞 "+(f.emergencyDefenseBuilt?"建設済み":"未建設");
  }
}
