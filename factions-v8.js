...
const newWarButton=$("newWarButton"),speedButton=$("speedButton"),placeNodeButton=$("placeNodeButton"),placementHint=$("placementHint");
...
let placementMode=false,manualNodePlaced=false,manualNodeType="SUPPLY";
...
function reset(){
  ...
  placementMode=false;manualNodePlaced=false;
  const types=Object.keys(NODE_TYPES);
  manualNodeType=types[Math.floor(Math.random()*types.length)];
  updatePlacementUI();
...
function updatePlacementUI(){
  if(!placeNodeButton)return;
  const nt=NODE_TYPES[manualNodeType];
  if(manualNodePlaced){
    placeNodeButton.disabled=true;
    placeNodeButton.classList?.remove("placing");
    placeNodeButton.querySelector("strong").textContent="自由拠点 配置済み";
    placeNodeButton.querySelector("span").textContent=nt.mark+"："+nt.label;
    if(placementHint)placementHint.hidden=true;
    return;
  }
  placeNodeButton.disabled=false;
  placeNodeButton.classList?.toggle("placing",placementMode);
  placeNodeButton.querySelector("strong").textContent=placementMode?"配置場所をクリック":"自由拠点を配置";
  placeNodeButton.querySelector("span").textContent="今回の能力："+nt.mark+" "+nt.label;
  if(placementHint){
    placementHint.hidden=!placementMode;
    placementHint.textContent="マップ上の好きな場所をクリック · 既存拠点や本拠点の近くには配置できません";
  }
}
function canPlaceManualNode(x,y){
  if(x<40||x>W-40||y<36||y>H-36)return false;
  for(const side of ["EMBER","AZURE"]){
    if(Math.hypot(x-factions[side].base.x,y-factions[side].base.y)<72)return false;
  }
  if(nodes.some(n=>Math.hypot(x-n.x,y-n.y)<58))return false;
  return true;
}
function placeManualNodeAt(x,y){
  if(manualNodePlaced||!placementMode)return false;
  if(!canPlaceManualNode(x,y)){
    log("自由拠点を配置できません · 既存拠点や本拠点から離してください");
    return false;
  }
  nodes.push({
    id:"F",x,y,type:manualNodeType,owner:null,capture:0,value:1,manual:true
  });
  manualNodePlaced=true;placementMode=false;updatePlacementUI();
  log("自由配置："+NODE_TYPES[manualNodeType].label+" を設置");
  return true;
}
...
function drawNodes(){
  for(const n of nodes){
    ...
    if(n.manual){ctx.strokeStyle="#ffffff";ctx.globalAlpha=.65;ctx.strokeRect(n.x-12,n.y-12,24,24);ctx.globalAlpha=1}
...
placeNodeButton.addEventListener("click",()=>{
  if(manualNodePlaced)return;
  placementMode=!placementMode;updatePlacementUI();
});
canvas.addEventListener("click",e=>{
  if(!placementMode||manualNodePlaced)return;
  const r=canvas.getBoundingClientRect();
  const x=(e.clientX-r.left)/Math.max(1,r.width)*W;
  const y=(e.clientY-r.top)/Math.max(1,r.height)*H;
  placeManualNodeAt(x,y);
});
